...
```{r}
deg.s4.res0.6 <- FindAllMarkers(SetIdent(s4, value = "SCT_snn_res.0.6"), only.pos = T, test.use = "t", logfc.threshold = 0.2) %>% mutate(diff = pct.1-pct.2)
```

save csv to file
```{r}
write_csv(deg.s4.res0.6, "data/tables/Stage4_res06_markers.csv")
```



```{r}
top <- deg.s4.res0.6 %>% dplyr::group_by(cluster) %>% top_n(3, diff)
top
```

```{r, fig.width = 16}
StrDotPlot(s4.sub, group.by = "SCT_snn_res.0.6", features = unique(c(top$gene)))
```

```{r}
p <- as.matrix(table(deg.s4.res0.6$gene, deg.s4.res0.6$cluster))
p <- cor(p)
```
...
## Section 2: off targets

take all the non-nephron clusters and perform a reanlysis of them alone.

```{r}
s4.nn <- s4[, s4$SCT_snn_res.0.1 %in% c(0,1,4,5,6,7,8)]
```


```{r}
DimPlot(s4.nn, group.by = "SCT_snn_res.0.6")
```


reanalyse these cells and look for different populatoins.

```{r}
s4.nn <- NormalizeData(s4.nn) %>% 
  ScaleData() %>% 
  CellCycleScoring(s.features = cc.genes.updated.2019$s.genes, g2m.features = cc.genes.updated.2019$g2m.genes)

s4.nn <- SCTransform(s4.nn, vars.to.regress = c("S.Score", "G2M.Score"), seed.use = 250395)

s4.nn <- RunPCA(s4.nn, npcs = 50, seed.use = 250395)
s4.nn <- RunUMAP(s4.nn, dims = 1:10, seed.use = 250395)
```
...
```{r}
DimPlot(s4.nn, group.by = "condition")
```

```{r}
s4.nn <- FindNeighbors(s4.nn)
s4.nn <- FindClusters(s4.nn, resolution = c(0.1, 0.3, 0.6, 0.9))

```

```{r, fig.width=16, fig.height=12}
MultiDimPlot(s4.nn, cluster.sequence = c("0.1", "0.3", "0.6", "0.9"))
```

```{r}
write_rds(s4.nn, "data/rds/Stage4_NonNeph.rds")
```


we had an older set of annotations, where do they sit?

```{r}
DimPlot(s4.nn, group.by = "ann", label = T, repel = T)
```



```{r}
s4.nn.deg <- FindAllMarkers(SetIdent(s4.nn, value = "SCT_snn_res.0.6"), only.pos = T, test.use = "t", logfc.threshold = 0.2) %>% mutate(diff = pct.1-pct.2)
...
```{r}
write_csv(s4.nn.deg, "data/tables/Stage4_NonNeph_markers.csv")
```


```{r}
top <- s4.nn.deg %>% dplyr::group_by(cluster) %>% top_n(4, diff)
```


```{r, fig.width=16}
StrDotPlot(s4.nn, group.by = "SCT_snn_res.0.6", features = unique(top$gene))
```

```{r, fig.width=16, fig.height=12}
FeaturePlot(s4.nn, features = c("PDGFRA", "PRRX1", "COL3A1", "COL9A1", # region 1 - mesenchymal
                                "MGP", "OGN", "STMN2", "ELAVL4", # region 1 - mesenchymal/chondrocyte and region 2 - glial
                                "MSX1", "S100B", "SOX10", "MLANA", # region 3 - neural crest glial and region 4 - neurons
                                "PITX2", "MYOG", "DNAJB1", "DNAJB4", # region 2 and 3 - neural and muscle
                                "SOX2", "ZIC2", "PAX3", "ZIC1"), # region 5 - less defined
            ncol = 4, order = T) & NoAxes()
```
...
```{r}
ComponentBar(s4.nn, "SCT_snn_res.0.6", "LineageID") + coord_flip()
ComponentBar(s4.nn, "SCT_snn_res.0.6", "LineageID", show.pct = T) + coord_flip()
ComponentBar(s4.nn, "SCT_snn_res.0.6", "ann", show.pct = T) + coord_flip()
```
...
ComponentHeatMapDF(s4.on.seu.sub, identity = "LineageID", component = "HDA_predicted.annotation.l2") %>% 
  mutate(Identity = factor(Identity, levels = names(DevKidCC::myColours()[unique(s4$LineageID)]))) %>% 
  ggplot() +
...
names(new.col) <- c(names(new.col)[1:20], 
                    "Astrocyte-like", "Glia-like", "Metanephric", "Stalled-NMP-MM", "Neural-like",
...
pop.types <- list(nephron = c("NPC", "EN", "PEC", "EPod", "Pod", "EPT", "PT", "EDT", "LOH", "DT", "UOS", "UTip", "UIS"),
                  ...
                  offtarget = c("Myocyte-like", "Satellite-cells", "PRRX1/2-PreMuscle", "Astrocyte-like", "Glia-like", "Neural-like", "Stalled-NP-MM"))
