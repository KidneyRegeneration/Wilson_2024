...
```{r}
png(res = 300, filename = "data/figures/Stage2_StrDotPlot_res05markers.png",units = "mm", width = 300, height = 75)
StrDotPlot(s2, group.by = "SCT_snn_res.0.3", features = unique(filters$gene))
dev.off()
```
...
colnames(counts.s2.c) <- paste0("Stage2_", colnames(counts.s2.c))
counts.ref.s2 <- bind_cols(ref.dge$counts[share.genes,], counts.s2.c[share.genes,])
...
StrDotPlot(s2, group.by = "SCT_snn_res.0.3",
           features = c("OSR1", "ITGA8", "HOXA10", "HOXD10", "HOXD11", "PAX2", "PAX8", "SALL1", "EYA1",
                        ...
                        "FOXF1",
                        "SOX2",
                        "TAGLN", "NEFM"), dot.scale = 10)
```

```{r}
png(res = 300, filename = "data/figures/Stage2_strdotplot_keygenes.png",units = "mm", width = 150, height = 80)
StrDotPlot(s2, group.by = "SCT_snn_res.0.3",
           features = c("SP5", "FGF8","TBX6", 
                        "OSR1", "ITGA8", "HOXA10", "HOXD10", "HOXD11", "PAX2", "PAX8", "SALL1", "EYA1",
                        "TCF15",
                        "PAX3", "SOX2",
                        "TAGLN", "NEFM",
                        "FOXF1"), dot.scale = 6)
...
s2ann <- s2@meta.data %>% mutate(SCT_snn_res.0.3 = as.character(SCT_snn_res.0.3)) %>%
  mutate(ann = case_when(SCT_snn_res.0.3 == "0" ~ "IM_1",
                         SCT_snn_res.0.3 == "1" ~ "NMP_IM_1",
                         SCT_snn_res.0.3 == "2" ~ "IM_2",
                         SCT_snn_res.0.3 == "3" ~ "PM_1",
                         SCT_snn_res.0.3 == "4" ~ "PM_2",
                         SCT_snn_res.0.3 == "5" ~ "IM_3",
                         SCT_snn_res.0.3 == "6" ~ "NMP_IM_2",
                         SCT_snn_res.0.3 == "7" ~ "NMP_1",
                         SCT_snn_res.0.3 == "8" ~ "PSM",
                         SCT_snn_res.0.3 == "9" ~ "NMP_2",
